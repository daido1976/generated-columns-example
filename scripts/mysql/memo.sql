...
-- Before
SELECT email FROM users WHERE email LIKE "%example.com" LIMIT 1000;

-- After
SELECT email FROM users WHERE email_domain = "example.com" LIMIT 1000;
...
-- After
SELECT email FROM users WHERE MATCH(email) AGAINST('*example.com' IN BOOLEAN MODE) LIMIT 1000;
...
-- Before
SELECT email FROM users WHERE email LIKE "%example.com" ORDER BY last_login_at LIMIT 1000;

-- After
SELECT email FROM users WHERE email_domain = "example.com" ORDER BY last_login_at LIMIT 1000;
